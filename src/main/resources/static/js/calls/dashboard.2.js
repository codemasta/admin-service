...
function runOptionsWidget() {
    ...
    $('.toggler-btn').on('click', function (e) {
        ...
        if (toggler.hasClass('open')) {
            ...
            subItem.velocity("stop").velocity("transition.slideDownOut", {
                stagger: 100,
                backwards: true,
                duration: speed
            });
            ...
            if (state) {
                mainItem.velocity("stop").velocity("transition.slideLeftIn", {
                    stagger: 100,
                    backwards: true,
                    duration: speed
                });
            } else {
                mainItem.velocity("stop").velocity("transition.slideRightIn", {
                    stagger: 100,
                    backwards: true,
                    duration: speed
                });
...
function runOptionsWidgetDataDisplay() {
    ...
    $('.details-btn').on('click', function (e) {
        ...
        if (infoChart.hasClass('open')) {
            ...
            infoChart.removeClass('open').velocity("stop").velocity("transition.flipYOut", {
                stagger: 100, backwards: true, duration: speed,
                complete: function (elements) {
                    infoData.addClass('open').velocity("stop").velocity("transition.flipYIn", {
                        stagger: 100,
                        backwards: true,
                        duration: speed
                    });
    ...
    $('.chart-btn').on('click', function (e) {
        ...
        if (infoData.hasClass('open')) {
            ...
            infoData.removeClass('open').velocity("stop").velocity("transition.flipYOut", {
                stagger: 100, backwards: true, duration: speed,
                complete: function () {
                    infoChart.addClass('open').velocity("stop").velocity("transition.flipYIn", {
                        stagger: 100, backwards: true, duration: speed,
                        complete: function () {
                            runCheckCharts();
                        }
...
$(function () {
    ...
    $(window).resize(function (e) {
        ...
        sparklineResize = setTimeout(function () {
            runCheckCharts()
        }, 500);
